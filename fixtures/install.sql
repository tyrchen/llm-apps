-- This code comes from, https://raw.githubusercontent.com/lorint/AdventureWorks-for-Postgres/. Please first download all csv following its instruction.
...
WITH RECURSIVE hier AS (
  ...
  FROM
    ...
    INNER JOIN hier ON e.businessentityid = hier.businessentityid
      AND i < LENGTH(e.org))
  UPDATE
    HumanResources.Employee AS emp
  SET
    org = COALESCE(trim(TRAILING '0' FROM hier.bits::text), '')
  FROM
    hier
WHERE
  emp.businessentityid = hier.businessentityid
  AND (hier.org IS NULL
    OR i = LENGTH(hier.org));
...
BEGIN
  LOOP
    ...
    UPDATE
      HumanResources.Employee
    SET
      organizationnode = organizationnode || SUBSTRING(org, 3, 2)::bit(2)::integer::varchar || CASE SUBSTRING(org, 5, 1)
      WHEN '0' THEN
        '.'
      ELSE
        '/'
    ...
    UPDATE
      HumanResources.Employee
    SET
      organizationnode = organizationnode || (SUBSTRING(org, 4, 2)::bit(2)::integer + 4)::varchar || CASE SUBSTRING(org, 6, 1)
      WHEN '0' THEN
        '.'
      ELSE
        '/'
    ...
    UPDATE
      HumanResources.Employee
    SET
      organizationnode = organizationnode || (SUBSTRING(org, 4, 3)::bit(3)::integer + 8)::varchar || CASE SUBSTRING(org, 7, 1)
      WHEN '0' THEN
        '.'
      ELSE
        '/'
    ...
    UPDATE
      HumanResources.Employee
    SET
      organizationnode = organizationnode || ((SUBSTRING(org, 4, 2) || SUBSTRING(org, 7, 1) || SUBSTRING(org, 9, 3))::bit(6)::integer + 16)::varchar || CASE SUBSTRING(org, 12, 1)
      WHEN '0' THEN
        '.'
      ELSE
        '/'
    ...
    UPDATE
      HumanResources.Employee
    SET
      organizationnode = organizationnode || ((SUBSTRING(org, 5, 3) || SUBSTRING(org, 9, 3) || SUBSTRING(org, 13, 1) || SUBSTRING(org, 15, 3))::bit(10)::integer + 80)::varchar || CASE SUBSTRING(org, 18, 1)
      WHEN '0' THEN
        '.'
      ELSE
        '/'
...
WITH RECURSIVE hier AS (
  ...
  SELECT
    ...
    i + 2 AS i
  FROM
    Production.Document AS e
    INNER JOIN hier ON e.rowguid = hier.rowguid
      AND i < LENGTH(e.doc))
  UPDATE
    Production.Document AS emp
  SET
    doc = COALESCE(trim(TRAILING '0' FROM hier.bits::text), '')
  FROM
    hier
WHERE
  emp.rowguid = hier.rowguid
  AND (hier.doc IS NULL
    OR i = LENGTH(hier.doc));
...
BEGIN
  LOOP
    ...
    UPDATE
      Production.Document
    SET
      DocumentNode = DocumentNode || SUBSTRING(doc, 3, 2)::bit(2)::integer::varchar || CASE SUBSTRING(doc, 5, 1)
      WHEN '0' THEN
        '.'
      ELSE
        '/'
    ...
    UPDATE
      Production.Document
    SET
      DocumentNode = DocumentNode || (SUBSTRING(doc, 4, 2)::bit(2)::integer + 4)::varchar || CASE SUBSTRING(doc, 6, 1)
      WHEN '0' THEN
        '.'
      ELSE
        '/'
    ...
    UPDATE
      Production.Document
    SET
      DocumentNode = DocumentNode || (SUBSTRING(doc, 4, 3)::bit(3)::integer + 8)::varchar || CASE SUBSTRING(doc, 7, 1)
      WHEN '0' THEN
        '.'
      ELSE
        '/'
    ...
    UPDATE
      Production.Document
    SET
      DocumentNode = DocumentNode || ((SUBSTRING(doc, 4, 2) || SUBSTRING(doc, 7, 1) || SUBSTRING(doc, 9, 3))::bit(6)::integer + 16)::varchar || CASE SUBSTRING(doc, 12, 1)
      WHEN '0' THEN
        '.'
      ELSE
        '/'
    ...
    UPDATE
      Production.Document
    SET
      DocumentNode = DocumentNode || ((SUBSTRING(doc, 5, 3) || SUBSTRING(doc, 9, 3) || SUBSTRING(doc, 13, 1) || SUBSTRING(doc, 15, 3))::bit(10)::integer + 80)::varchar || CASE SUBSTRING(doc, 18, 1)
      WHEN '0' THEN
        '.'
      ELSE
        '/'
...
ALTER TABLE Production.ProductDocument
  ADD rowguid uuid NOT NULL CONSTRAINT "DF_ProductDocument_rowguid" DEFAULT (uuid_generate_v1 ());
...
WITH RECURSIVE hier AS (
  ...
  SELECT
    ...
    i + 2 AS i
  FROM
    Production.ProductDocument AS e
    INNER JOIN hier ON e.rowguid = hier.rowguid
      AND i < LENGTH(e.doc))
  UPDATE
    Production.ProductDocument AS emp
  SET
    doc = COALESCE(trim(TRAILING '0' FROM hier.bits::text), '')
  FROM
    hier
...
SELECT
  'Copying data into Purchasing.ProductVendor';
...
SELECT
  'Copying data into Purchasing.Vendor';
...
SELECT
  'Copying data into Sales.CountryRegionCurrency';
...
CLUSTER Person.Address
USING "PK_Address_AddressID";

ALTER TABLE Person.AddressType
  ADD CONSTRAINT "PK_AddressType_AddressTypeID" PRIMARY KEY (AddressTypeID);

CLUSTER Person.AddressType
USING "PK_AddressType_AddressTypeID";
...
CLUSTER Person.BusinessEntity
USING "PK_BusinessEntity_BusinessEntityID";
...
CLUSTER Person.BusinessEntityAddress
USING "PK_BusinessEntityAddress_BusinessEntityID_AddressID_AddressType";
...
CLUSTER Person.BusinessEntityContact
USING "PK_BusinessEntityContact_BusinessEntityID_PersonID_ContactTypeI";

ALTER TABLE Person.ContactType
  ADD CONSTRAINT "PK_ContactType_ContactTypeID" PRIMARY KEY (ContactTypeID);

CLUSTER Person.ContactType
USING "PK_ContactType_ContactTypeID";
...
CLUSTER HumanResources.EmployeeDepartmentHistory
USING "PK_EmployeeDepartmentHistory_BusinessEntityID_StartDate_Departm";
...
CLUSTER Production.ProductDescription
USING "PK_ProductDescription_ProductDescriptionID";

ALTER TABLE Production.ProductDocument
  ADD CONSTRAINT "PK_ProductDocument_ProductID_DocumentNode" PRIMARY KEY (ProductID, DocumentNode);

CLUSTER Production.ProductDocument
USING "PK_ProductDocument_ProductID_DocumentNode";

ALTER TABLE Production.ProductInventory
  ADD CONSTRAINT "PK_ProductInventory_ProductID_LocationID" PRIMARY KEY (ProductID, LocationID);

CLUSTER Production.ProductInventory
USING "PK_ProductInventory_ProductID_LocationID";
...
CLUSTER Production.ProductListPriceHistory
USING "PK_ProductListPriceHistory_ProductID_StartDate";

ALTER TABLE Production.ProductModel
  ADD CONSTRAINT "PK_ProductModel_ProductModelID" PRIMARY KEY (ProductModelID);

CLUSTER Production.ProductModel
USING "PK_ProductModel_ProductModelID";
...
CLUSTER Production.ProductModelIllustration
USING "PK_ProductModelIllustration_ProductModelID_IllustrationID";
...
CLUSTER Production.ProductModelProductDescriptionCulture
USING "PK_ProductModelProductDescriptionCulture_ProductModelID_Product";

ALTER TABLE Production.ProductPhoto
  ADD CONSTRAINT "PK_ProductPhoto_ProductPhotoID" PRIMARY KEY (ProductPhotoID);

CLUSTER Production.ProductPhoto
USING "PK_ProductPhoto_ProductPhotoID";
...
CLUSTER Production.ProductReview
USING "PK_ProductReview_ProductReviewID";
...
CLUSTER Production.ProductSubcategory
USING "PK_ProductSubcategory_ProductSubcategoryID";
...
CLUSTER Purchasing.ProductVendor
USING "PK_ProductVendor_ProductID_BusinessEntityID";
...
CLUSTER Purchasing.PurchaseOrderDetail
USING "PK_PurchaseOrderDetail_PurchaseOrderID_PurchaseOrderDetailID";

ALTER TABLE Purchasing.PurchaseOrderHeader
  ADD CONSTRAINT "PK_PurchaseOrderHeader_PurchaseOrderID" PRIMARY KEY (PurchaseOrderID);

CLUSTER Purchasing.PurchaseOrderHeader
USING "PK_PurchaseOrderHeader_PurchaseOrderID";
...
CLUSTER Sales.PersonCreditCard
USING "PK_PersonCreditCard_BusinessEntityID_CreditCardID";

ALTER TABLE Sales.SalesOrderDetail
  ADD CONSTRAINT "PK_SalesOrderDetail_SalesOrderID_SalesOrderDetailID" PRIMARY KEY (SalesOrderID, SalesOrderDetailID);

CLUSTER Sales.SalesOrderDetail
USING "PK_SalesOrderDetail_SalesOrderID_SalesOrderDetailID";

ALTER TABLE Sales.SalesOrderHeader
  ADD CONSTRAINT "PK_SalesOrderHeader_SalesOrderID" PRIMARY KEY (SalesOrderID);

CLUSTER Sales.SalesOrderHeader
USING "PK_SalesOrderHeader_SalesOrderID";
...
CLUSTER Sales.SalesOrderHeaderSalesReason
USING "PK_SalesOrderHeaderSalesReason_SalesOrderID_SalesReasonID";

ALTER TABLE Sales.SalesPerson
  ADD CONSTRAINT "PK_SalesPerson_BusinessEntityID" PRIMARY KEY (BusinessEntityID);

CLUSTER Sales.SalesPerson
USING "PK_SalesPerson_BusinessEntityID";
...
-- ProductCategoryID);
CLUSTER Sales.SalesPersonQuotaHistory
...
CLUSTER Production.WorkOrderRouting
USING "PK_WorkOrderRouting_WorkOrderID_ProductID_OperationSequence";
...
SELECT
  ...
  p.MiddleName,
  p.LastName,
  (xpath('(act:telephoneNumber)[1]/act:number/text()', node, '{{act,http://schemas.microsoft.com/sqlserver/2004/07/adventure-works/ContactTypes}}'))[1] AS TelephoneNumber,
  BTRIM((xpath('(act:telephoneNumber)[1]/act:SpecialInstructions/text()', node, '{{act,http://schemas.microsoft.com/sqlserver/2004/07/adventure-works/ContactTypes}}'))[1]::varchar) AS TelephoneSpecialInstructions,
  (xpath('(act:homePostalAddress)[1]/act:Street/text()', node, '{{act,http://schemas.microsoft.com/sqlserver/2004/07/adventure-works/ContactTypes}}'))[1] AS Street,
  (xpath('(act:homePostalAddress)[1]/act:City/text()', node, '{{act,http://schemas.microsoft.com/sqlserver/2004/07/adventure-works/ContactTypes}}'))[1] AS City,
  (xpath('(act:homePostalAddress)[1]/act:StateProvince/text()', node, '{{act,http://schemas.microsoft.com/sqlserver/2004/07/adventure-works/ContactTypes}}'))[1] AS StateProvince,
  (xpath('(act:homePostalAddress)[1]/act:PostalCode/text()', node, '{{act,http://schemas.microsoft.com/sqlserver/2004/07/adventure-works/ContactTypes}}'))[1] AS PostalCode,
  (xpath('(act:homePostalAddress)[1]/act:CountryRegion/text()', node, '{{act,http://schemas.microsoft.com/sqlserver/2004/07/adventure-works/ContactTypes}}'))[1] AS CountryRegion,
  (xpath('(act:homePostalAddress)[1]/act:SpecialInstructions/text()', node, '{{act,http://schemas.microsoft.com/sqlserver/2004/07/adventure-works/ContactTypes}}'))[1] AS HomeAddressSpecialInstructions,
  (xpath('(act:eMail)[1]/act:eMailAddress/text()', node, '{{act,http://schemas.microsoft.com/sqlserver/2004/07/adventure-works/ContactTypes}}'))[1] AS EMailAddress,
  BTRIM((xpath('(act:eMail)[1]/act:SpecialInstructions/text()', node, '{{act,http://schemas.microsoft.com/sqlserver/2004/07/adventure-works/ContactTypes}}'))[1]::varchar) AS EMailSpecialInstructions,
  (xpath('((act:eMail)[1]/act:SpecialInstructions/act:telephoneNumber)[1]/act:number/text()', node, '{{act,http://schemas.microsoft.com/sqlserver/2004/07/adventure-works/ContactTypes}}'))[1] AS EMailTelephoneNumber,
  p.rowguid,
  p.ModifiedDate
FROM
  Person.Person AS p
  ...
  SELECT
    BusinessEntityID,
    UNNEST(xpath('/ci:AdditionalContactInfo', additionalcontactinfo, '{{ci,http://schemas.microsoft.com/sqlserver/2004/07/adventure-works/ContactInfo}}')) AS node
  FROM
    Person.Person
  WHERE
    AdditionalContactInfo IS NOT NULL) AS additional ON p.BusinessEntityID = additional.BusinessEntityID;
...
SELECT
  BusinessEntityID,
  CAST((xpath('n:TotalPurchaseYTD/text()', Demographics, '{{n,http://schemas.microsoft.com/sqlserver/2004/07/adventure-works/IndividualSurvey}}'))[1]::varchar AS money) AS TotalPurchaseYTD,
  CAST((xpath('n:DateFirstPurchase/text()', Demographics, '{{n,http://schemas.microsoft.com/sqlserver/2004/07/adventure-works/IndividualSurvey}}'))[1]::varchar AS DATE) AS DateFirstPurchase,
  CAST((xpath('n:BirthDate/text()', Demographics, '{{n,http://schemas.microsoft.com/sqlserver/2004/07/adventure-works/IndividualSurvey}}'))[1]::varchar AS DATE) AS BirthDate,
  (xpath('n:MaritalStatus/text()', Demographics, '{{n,http://schemas.microsoft.com/sqlserver/2004/07/adventure-works/IndividualSurvey}}'))[1]::varchar(1) AS MaritalStatus,
  (xpath('n:YearlyIncome/text()', Demographics, '{{n,http://schemas.microsoft.com/sqlserver/2004/07/adventure-works/IndividualSurvey}}'))[1]::varchar(30) AS YearlyIncome,
  (xpath('n:Gender/text()', Demographics, '{{n,http://schemas.microsoft.com/sqlserver/2004/07/adventure-works/IndividualSurvey}}'))[1]::varchar(1) AS Gender,
  CAST((xpath('n:TotalChildren/text()', Demographics, '{{n,http://schemas.microsoft.com/sqlserver/2004/07/adventure-works/IndividualSurvey}}'))[1]::varchar AS INTEGER) AS TotalChildren,
  CAST((xpath('n:NumberChildrenAtHome/text()', Demographics, '{{n,http://schemas.microsoft.com/sqlserver/2004/07/adventure-works/IndividualSurvey}}'))[1]::varchar AS INTEGER) AS NumberChildrenAtHome,
  (xpath('n:Education/text()', Demographics, '{{n,http://schemas.microsoft.com/sqlserver/2004/07/adventure-works/IndividualSurvey}}'))[1]::varchar(30) AS Education,
  (xpath('n:Occupation/text()', Demographics, '{{n,http://schemas.microsoft.com/sqlserver/2004/07/adventure-works/IndividualSurvey}}'))[1]::varchar(30) AS Occupation,
  CAST((xpath('n:HomeOwnerFlag/text()', Demographics, '{{n,http://schemas.microsoft.com/sqlserver/2004/07/adventure-works/IndividualSurvey}}'))[1]::varchar AS BOOLEAN) AS HomeOwnerFlag,
  CAST((xpath('n:NumberCarsOwned/text()', Demographics, '{{n,http://schemas.microsoft.com/sqlserver/2004/07/adventure-works/IndividualSurvey}}'))[1]::varchar AS INTEGER) AS NumberCarsOwned
FROM
  Person.Person
WHERE
...
SELECT
  JobCandidateID,
  BusinessEntityID,
  (xpath('/n:Resume/n:Name/n:Name.Prefix/text()', Resume, '{{n,http://schemas.microsoft.com/sqlserver/2004/07/adventure-works/Resume}}'))[1]::varchar(30) AS "Name.Prefix",
  (xpath('/n:Resume/n:Name/n:Name.First/text()', Resume, '{{n,http://schemas.microsoft.com/sqlserver/2004/07/adventure-works/Resume}}'))[1]::varchar(30) AS "Name.First",
  (xpath('/n:Resume/n:Name/n:Name.Middle/text()', Resume, '{{n,http://schemas.microsoft.com/sqlserver/2004/07/adventure-works/Resume}}'))[1]::varchar(30) AS "Name.Middle",
  (xpath('/n:Resume/n:Name/n:Name.Last/text()', Resume, '{{n,http://schemas.microsoft.com/sqlserver/2004/07/adventure-works/Resume}}'))[1]::varchar(30) AS "Name.Last",
  (xpath('/n:Resume/n:Name/n:Name.Suffix/text()', Resume, '{{n,http://schemas.microsoft.com/sqlserver/2004/07/adventure-works/Resume}}'))[1]::varchar(30) AS "Name.Suffix",
  (xpath('/n:Resume/n:Skills/text()', Resume, '{{n,http://schemas.microsoft.com/sqlserver/2004/07/adventure-works/Resume}}'))[1]::varchar AS "Skills",
  (xpath('n:Address/n:Addr.Type/text()', Resume, '{{n,http://schemas.microsoft.com/sqlserver/2004/07/adventure-works/Resume}}'))[1]::varchar(30) AS "Addr.Type",
  (xpath('n:Address/n:Addr.Location/n:Location/n:Loc.CountryRegion/text()', Resume, '{{n,http://schemas.microsoft.com/sqlserver/2004/07/adventure-works/Resume}}'))[1]::varchar(100) AS "Addr.Loc.CountryRegion",
  (xpath('n:Address/n:Addr.Location/n:Location/n:Loc.State/text()', Resume, '{{n,http://schemas.microsoft.com/sqlserver/2004/07/adventure-works/Resume}}'))[1]::varchar(100) AS "Addr.Loc.State",
  (xpath('n:Address/n:Addr.Location/n:Location/n:Loc.City/text()', Resume, '{{n,http://schemas.microsoft.com/sqlserver/2004/07/adventure-works/Resume}}'))[1]::varchar(100) AS "Addr.Loc.City",
  (xpath('n:Address/n:Addr.PostalCode/text()', Resume, '{{n,http://schemas.microsoft.com/sqlserver/2004/07/adventure-works/Resume}}'))[1]::varchar(20) AS "Addr.PostalCode",
  (xpath('/n:Resume/n:EMail/text()', Resume, '{{n,http://schemas.microsoft.com/sqlserver/2004/07/adventure-works/Resume}}'))[1]::varchar AS "EMail",
  (xpath('/n:Resume/n:WebSite/text()', Resume, '{{n,http://schemas.microsoft.com/sqlserver/2004/07/adventure-works/Resume}}'))[1]::varchar AS "WebSite",
  ModifiedDate
FROM
...
SELECT
  JobCandidateID,
  CAST(UNNEST(xpath('/ns:Resume/ns:Employment/ns:Emp.StartDate/text()', Resume, '{{ns,http://schemas.microsoft.com/sqlserver/2004/07/adventure-works/Resume}}'))::varchar(20) AS DATE) AS "Emp.StartDate",
  CAST(UNNEST(xpath('/ns:Resume/ns:Employment/ns:Emp.EndDate/text()', Resume, '{{ns,http://schemas.microsoft.com/sqlserver/2004/07/adventure-works/Resume}}'))::varchar(20) AS DATE) AS "Emp.EndDate",
  UNNEST(xpath('/ns:Resume/ns:Employment/ns:Emp.OrgName/text()', Resume, '{{ns,http://schemas.microsoft.com/sqlserver/2004/07/adventure-works/Resume}}'))::varchar(100) AS "Emp.OrgName",
  UNNEST(xpath('/ns:Resume/ns:Employment/ns:Emp.JobTitle/text()', Resume, '{{ns,http://schemas.microsoft.com/sqlserver/2004/07/adventure-works/Resume}}'))::varchar(100) AS "Emp.JobTitle",
  UNNEST(xpath('/ns:Resume/ns:Employment/ns:Emp.Responsibility/text()', Resume, '{{ns,http://schemas.microsoft.com/sqlserver/2004/07/adventure-works/Resume}}'))::varchar AS "Emp.Responsibility",
  UNNEST(xpath('/ns:Resume/ns:Employment/ns:Emp.FunctionCategory/text()', Resume, '{{ns,http://schemas.microsoft.com/sqlserver/2004/07/adventure-works/Resume}}'))::varchar AS "Emp.FunctionCategory",
  UNNEST(xpath('/ns:Resume/ns:Employment/ns:Emp.IndustryCategory/text()', Resume, '{{ns,http://schemas.microsoft.com/sqlserver/2004/07/adventure-works/Resume}}'))::varchar AS "Emp.IndustryCategory",
  UNNEST(xpath('/ns:Resume/ns:Employment/ns:Emp.Location/ns:Location/ns:Loc.CountryRegion/text()', Resume, '{{ns,http://schemas.microsoft.com/sqlserver/2004/07/adventure-works/Resume}}'))::varchar AS "Emp.Loc.CountryRegion",
  UNNEST(xpath('/ns:Resume/ns:Employment/ns:Emp.Location/ns:Location/ns:Loc.State/text()', Resume, '{{ns,http://schemas.microsoft.com/sqlserver/2004/07/adventure-works/Resume}}'))::varchar AS "Emp.Loc.State",
  UNNEST(xpath('/ns:Resume/ns:Employment/ns:Emp.Location/ns:Location/ns:Loc.City/text()', Resume, '{{ns,http://schemas.microsoft.com/sqlserver/2004/07/adventure-works/Resume}}'))::varchar AS "Emp.Loc.City"
FROM
  HumanResources.JobCandidate;
...
SELECT
  jc.JobCandidateID,
  (xpath('/root/ns:Education/ns:Edu.Level/text()', jc.doc, '{{ns,http://adventureworks.com}}'))[1]::varchar(50) AS "Edu.Level",
  CAST((xpath('/root/ns:Education/ns:Edu.StartDate/text()', jc.doc, '{{ns,http://adventureworks.com}}'))[1]::varchar(20) AS DATE) AS "Edu.StartDate",
  CAST((xpath('/root/ns:Education/ns:Edu.EndDate/text()', jc.doc, '{{ns,http://adventureworks.com}}'))[1]::varchar(20) AS DATE) AS "Edu.EndDate",
  (xpath('/root/ns:Education/ns:Edu.Degree/text()', jc.doc, '{{ns,http://adventureworks.com}}'))[1]::varchar(50) AS "Edu.Degree",
  (xpath('/root/ns:Education/ns:Edu.Major/text()', jc.doc, '{{ns,http://adventureworks.com}}'))[1]::varchar(50) AS "Edu.Major",
  (xpath('/root/ns:Education/ns:Edu.Minor/text()', jc.doc, '{{ns,http://adventureworks.com}}'))[1]::varchar(50) AS "Edu.Minor",
  (xpath('/root/ns:Education/ns:Edu.GPA/text()', jc.doc, '{{ns,http://adventureworks.com}}'))[1]::varchar(5) AS "Edu.GPA",
  (xpath('/root/ns:Education/ns:Edu.GPAScale/text()', jc.doc, '{{ns,http://adventureworks.com}}'))[1]::varchar(5) AS "Edu.GPAScale",
  (xpath('/root/ns:Education/ns:Edu.School/text()', jc.doc, '{{ns,http://adventureworks.com}}'))[1]::varchar(100) AS "Edu.School",
  (xpath('/root/ns:Education/ns:Edu.Location/ns:Location/ns:Loc.CountryRegion/text()', jc.doc, '{{ns,http://adventureworks.com}}'))[1]::varchar(100) AS "Edu.Loc.CountryRegion",
  (xpath('/root/ns:Education/ns:Edu.Location/ns:Location/ns:Loc.State/text()', jc.doc, '{{ns,http://adventureworks.com}}'))[1]::varchar(100) AS "Edu.Loc.State",
  (xpath('/root/ns:Education/ns:Edu.Location/ns:Location/ns:Loc.City/text()', jc.doc, '{{ns,http://adventureworks.com}}'))[1]::varchar(100) AS "Edu.Loc.City"
FROM (
  SELECT
    JobCandidateID
...
,
    ...
    ('<root xmlns:ns="http://adventureworks.com">' || unnesting.Education::varchar || '</root>')::xml AS doc
  FROM (
    SELECT
      JobCandidateID,
    ...
    FROM
      HumanResources.JobCandidate) AS unnesting) AS jc;
...
CLUSTER Production.vProductAndDescription
USING IX_vProductAndDescription;
...
SELECT
  ProductModelID,
  Name,
  (xpath('/p1:ProductDescription/p1:Summary/html:p/text()', CatalogDescription, '{{p1,http://schemas.microsoft.com/sqlserver/2004/07/adventure-works/ProductModelDescription},{html,http://www.w3.org/1999/xhtml}}'))[1]::varchar AS "Summary",
  (xpath('/p1:ProductDescription/p1:Manufacturer/p1:Name/text()', CatalogDescription, '{{p1,http://schemas.microsoft.com/sqlserver/2004/07/adventure-works/ProductModelDescription}}'))[1]::varchar AS Manufacturer,
  (xpath('/p1:ProductDescription/p1:Manufacturer/p1:Copyright/text()', CatalogDescription, '{{p1,http://schemas.microsoft.com/sqlserver/2004/07/adventure-works/ProductModelDescription}}'))[1]::varchar(30) AS Copyright,
  (xpath('/p1:ProductDescription/p1:Manufacturer/p1:ProductURL/text()', CatalogDescription, '{{p1,http://schemas.microsoft.com/sqlserver/2004/07/adventure-works/ProductModelDescription}}'))[1]::varchar(256) AS ProductURL,
  (xpath('/p1:ProductDescription/p1:Features/wm:Warranty/wm:WarrantyPeriod/text()', CatalogDescription, '{{p1,http://schemas.microsoft.com/sqlserver/2004/07/adventure-works/ProductModelDescription},{wm,http://schemas.microsoft.com/sqlserver/2004/07/adventure-works/ProductModelWarrAndMain}}'))[1]::varchar(256) AS WarrantyPeriod,
  (xpath('/p1:ProductDescription/p1:Features/wm:Warranty/wm:Description/text()', CatalogDescription, '{{p1,http://schemas.microsoft.com/sqlserver/2004/07/adventure-works/ProductModelDescription},{wm,http://schemas.microsoft.com/sqlserver/2004/07/adventure-works/ProductModelWarrAndMain}}'))[1]::varchar(256) AS WarrantyDescription,
  (xpath('/p1:ProductDescription/p1:Features/wm:Maintenance/wm:NoOfYears/text()', CatalogDescription, '{{p1,http://schemas.microsoft.com/sqlserver/2004/07/adventure-works/ProductModelDescription},{wm,http://schemas.microsoft.com/sqlserver/2004/07/adventure-works/ProductModelWarrAndMain}}'))[1]::varchar(256) AS NoOfYears,
  (xpath('/p1:ProductDescription/p1:Features/wm:Maintenance/wm:Description/text()', CatalogDescription, '{{p1,http://schemas.microsoft.com/sqlserver/2004/07/adventure-works/ProductModelDescription},{wm,http://schemas.microsoft.com/sqlserver/2004/07/adventure-works/ProductModelWarrAndMain}}'))[1]::varchar(256) AS MaintenanceDescription,
  (xpath('/p1:ProductDescription/p1:Features/wf:wheel/text()', CatalogDescription, '{{p1,http://schemas.microsoft.com/sqlserver/2004/07/adventure-works/ProductModelDescription},{wf,http://www.adventure-works.com/schemas/OtherFeatures}}'))[1]::varchar(256) AS Wheel,
  (xpath('/p1:ProductDescription/p1:Features/wf:saddle/text()', CatalogDescription, '{{p1,http://schemas.microsoft.com/sqlserver/2004/07/adventure-works/ProductModelDescription},{wf,http://www.adventure-works.com/schemas/OtherFeatures}}'))[1]::varchar(256) AS Saddle,
  (xpath('/p1:ProductDescription/p1:Features/wf:pedal/text()', CatalogDescription, '{{p1,http://schemas.microsoft.com/sqlserver/2004/07/adventure-works/ProductModelDescription},{wf,http://www.adventure-works.com/schemas/OtherFeatures}}'))[1]::varchar(256) AS Pedal,
  (xpath('/p1:ProductDescription/p1:Features/wf:BikeFrame/text()', CatalogDescription, '{{p1,http://schemas.microsoft.com/sqlserver/2004/07/adventure-works/ProductModelDescription},{wf,http://www.adventure-works.com/schemas/OtherFeatures}}'))[1]::varchar AS BikeFrame,
  (xpath('/p1:ProductDescription/p1:Features/wf:crankset/text()', CatalogDescription, '{{p1,http://schemas.microsoft.com/sqlserver/2004/07/adventure-works/ProductModelDescription},{wf,http://www.adventure-works.com/schemas/OtherFeatures}}'))[1]::varchar(256) AS Crankset,
  (xpath('/p1:ProductDescription/p1:Picture/p1:Angle/text()', CatalogDescription, '{{p1,http://schemas.microsoft.com/sqlserver/2004/07/adventure-works/ProductModelDescription}}'))[1]::varchar(256) AS PictureAngle,
  (xpath('/p1:ProductDescription/p1:Picture/p1:Size/text()', CatalogDescription, '{{p1,http://schemas.microsoft.com/sqlserver/2004/07/adventure-works/ProductModelDescription}}'))[1]::varchar(256) AS PictureSize,
  (xpath('/p1:ProductDescription/p1:Picture/p1:ProductPhotoID/text()', CatalogDescription, '{{p1,http://schemas.microsoft.com/sqlserver/2004/07/adventure-works/ProductModelDescription}}'))[1]::varchar(256) AS ProductPhotoID,
  (xpath('/p1:ProductDescription/p1:Specifications/Material/text()', CatalogDescription, '{{p1,http://schemas.microsoft.com/sqlserver/2004/07/adventure-works/ProductModelDescription}}'))[1]::varchar(256) AS Material,
  (xpath('/p1:ProductDescription/p1:Specifications/Color/text()', CatalogDescription, '{{p1,http://schemas.microsoft.com/sqlserver/2004/07/adventure-works/ProductModelDescription}}'))[1]::varchar(256) AS Color,
  (xpath('/p1:ProductDescription/p1:Specifications/ProductLine/text()', CatalogDescription, '{{p1,http://schemas.microsoft.com/sqlserver/2004/07/adventure-works/ProductModelDescription}}'))[1]::varchar(256) AS ProductLine,
  (xpath('/p1:ProductDescription/p1:Specifications/Style/text()', CatalogDescription, '{{p1,http://schemas.microsoft.com/sqlserver/2004/07/adventure-works/ProductModelDescription}}'))[1]::varchar(256) AS Style,
  (xpath('/p1:ProductDescription/p1:Specifications/RiderExperience/text()', CatalogDescription, '{{p1,http://schemas.microsoft.com/sqlserver/2004/07/adventure-works/ProductModelDescription}}'))[1]::varchar(1024) AS RiderExperience,
  rowguid,
  ModifiedDate
FROM
...
FROM (
  ...
  FROM (
    ...
    FROM
      Production.ProductModel) AS locations) AS pm;
...
FROM
  ...
  (
    ...
    FROM
      ...
      INNER JOIN Person.Person p ON p.BusinessEntityID = sp.BusinessEntityID) AS granular
GROUP BY
  granular.SalesPersonID,
  granular.FullName,
  granular.JobTitle,
...
ORDER BY 2,4',
    ...
    $$
    SELECT
      unnest('{2012,2013,2014}'::text[]) $$)
  -- ... still the FiscalYear values have to be hard-coded here.
  AS SalesTotal ("SalesPersonID" integer,
    "FullName" text,
    "JobTitle" text,
    "SalesTerritory" text,
    "2012" DECIMAL(12, 4),
...
CLUSTER Person.vStateProvinceCountryRegion
USING IX_vStateProvinceCountryRegion;
...
SELECT
  BusinessEntityID,
  Name,
  CAST(UNNEST(xpath('/ns:StoreSurvey/ns:AnnualSales/text()', Demographics, '{{ns,http://schemas.microsoft.com/sqlserver/2004/07/adventure-works/StoreSurvey}}'))::varchar AS money) AS "AnnualSales",
  CAST(UNNEST(xpath('/ns:StoreSurvey/ns:AnnualRevenue/text()', Demographics, '{{ns,http://schemas.microsoft.com/sqlserver/2004/07/adventure-works/StoreSurvey}}'))::varchar AS money) AS "AnnualRevenue",
  UNNEST(xpath('/ns:StoreSurvey/ns:BankName/text()', Demographics, '{{ns,http://schemas.microsoft.com/sqlserver/2004/07/adventure-works/StoreSurvey}}'))::varchar(50) AS "BankName",
  UNNEST(xpath('/ns:StoreSurvey/ns:BusinessType/text()', Demographics, '{{ns,http://schemas.microsoft.com/sqlserver/2004/07/adventure-works/StoreSurvey}}'))::varchar(5) AS "BusinessType",
  CAST(UNNEST(xpath('/ns:StoreSurvey/ns:YearOpened/text()', Demographics, '{{ns,http://schemas.microsoft.com/sqlserver/2004/07/adventure-works/StoreSurvey}}'))::varchar AS integer) AS "YearOpened",
  UNNEST(xpath('/ns:StoreSurvey/ns:Specialty/text()', Demographics, '{{ns,http://schemas.microsoft.com/sqlserver/2004/07/adventure-works/StoreSurvey}}'))::varchar(50) AS "Specialty",
  CAST(UNNEST(xpath('/ns:StoreSurvey/ns:SquareFeet/text()', Demographics, '{{ns,http://schemas.microsoft.com/sqlserver/2004/07/adventure-works/StoreSurvey}}'))::varchar AS integer) AS "SquareFeet",
  UNNEST(xpath('/ns:StoreSurvey/ns:Brands/text()', Demographics, '{{ns,http://schemas.microsoft.com/sqlserver/2004/07/adventure-works/StoreSurvey}}'))::varchar(30) AS "Brands",
  UNNEST(xpath('/ns:StoreSurvey/ns:Internet/text()', Demographics, '{{ns,http://schemas.microsoft.com/sqlserver/2004/07/adventure-works/StoreSurvey}}'))::varchar(30) AS "Internet",
  CAST(UNNEST(xpath('/ns:StoreSurvey/ns:NumberEmployees/text()', Demographics, '{{ns,http://schemas.microsoft.com/sqlserver/2004/07/adventure-works/StoreSurvey}}'))::varchar AS integer) AS "NumberEmployees"
FROM
  Sales.Store;
...
CREATE SCHEMA sa
  ...
  CREATE VIEW sci AS
  SELECT
    shoppingcartitemid AS id,
    *
  FROM
    sales.shoppingcartitem
  CREATE VIEW so AS
  SELECT
    specialofferid AS id,
    *
  FROM
    sales.specialoffer
  CREATE VIEW sop AS
